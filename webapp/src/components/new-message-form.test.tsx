...
describe("NewMessageForm view", () => {
  ...
  it("Calls onSend with body and file", () => {
    ...
    const { getByText } = render(
      <NewMessageFormView controllerBusy={false} onSend={onSend} />,
    );
...
describe("NewMessageForm controller", () => {
  it("Sends message only", () => {
    const DummyComponent = ({
      ...
      useEffect(() => {
        if (!controllerBusy) onSend("test", null);
      });
      return <div></div>;
    };
    const chatHandler = {
    ...
    waitFor(() =>
      expect(chatHandler.addNewMessage).toHaveBeenCalledWith("test", null),
    );
  });
